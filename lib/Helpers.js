...
/**
 * Update listings last retrieved timestamp
 */
Helpers.touchListingsLastRetrieved = function () {
  var timestamp = (new Date()).getTime();

  Session.setTemp('listings_last_retrieved_timestamp', timestamp);
}
 ...
 * @return {Object}
 */
Helpers.findChatByParticipants = function (participants) {
  var and = [];

  _.each(participants, function (participant) {
    and.push({ participants: participant });
  });

  return Chats.findOne({ $and: and });
}
